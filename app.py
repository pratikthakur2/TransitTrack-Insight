...
def display(filename):
    ...
    #check if list of subfolders is empty
    if not subfolders:
        return "No subfolders found"
    
    #filter out empty subfolders
    non_empty_subfolders = [subfolder for subfolder in subfolders if os.listdir(os.path.join(folder_path,subfolder))]
    
    #check if there are any non-empty subfolders
    if not non_empty_subfolders:
        return "No non-empty subfolders found"

    
    
    #get the latest non-empty subfolder
    latest_subfolder = max(non_empty_subfolders, key=lambda x: os.path.getctime(os.path.join(folder_path, x)))
